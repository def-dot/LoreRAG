...
// 知识库检索
export function searchKnowledge(query: string, topK = 5, mode = 'hybrid', useLLM = false) {
  return request.post<{ code: number; msg: string; data: { results: SearchResult[]; total: number; tokens: string[]; answer: string } }>(
    '/rag/search',
    { query, top_k: topK, mode, use_llm: useLLM },
  )
}
