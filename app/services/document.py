...
def process_document(file_path: str) -> list[dict[str, Any]]:
        ...
        if element.label == "figure":
            if hasattr(element, "image") and element.image is not None:
                ...
                try:
                    ...
                    element.text = f"{element.text}\n{description}" if element.text else description
                finally:
                    if os.path.exists(tmp_path):
                        os.remove(tmp_path)

    # HybridChunker 智能层级切片（保证表格/公式不被截断）
    chunker = HybridChunker(max_tokens=settings.CHUNK_SIZE, overlap_tokens=settings.CHUNK_OVERLAP)
    ...
    for i, chunk in enumerate(doc_chunks):
        ...
        heading_ctx = " > ".join(h.text for h in headings) if headings else "Root"
        pages = list(chunk.meta.page_numbers) if chunk.meta.page_numbers else [1]
